...
GYPKG.prototype.installDep = function installDep(uri, gyp, target, callback) {
  ...
  async.waterfall([
    ...
    (callback) => {
      ...
      this.checkoutSemVer(INSTALL_DIR, version, callback);
    }
  ], (err, depDir) => {
    if (err)
      return callback(err);

    callback(null, path.join(depDir, gyp) + ':' + target);
  });
};
...
GYPKG.prototype.git = function git(args, options, callback) {
  ...
  git.on('exit', (code) => {
    if (code !== 0) {
      // Retry if locked
      if (/Another git process/i.test(stderr)) {
...
GYPKG.prototype.checkoutSemVer = function checkoutSemVer(target, version,
                                                         callback) {
  ...
  async.waterfall([
    ...
    (tags, callback) => {
      ...
      if (tags.length === 0) {
        return callback(
            new Error(`No matching version found, ${target}:${version}`));
      }
