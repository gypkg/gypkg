...
function GYPKG(options) {
  this.options = options;

  this.GYPKG_DEPS = process.env.GYPKG_DEPS;
  this.GYPKG_LOG_HOST = process.env.GYPKG_LOG_HOST || '127.0.0.1';
  this.GYPKG_LOG_PORT = process.env.GYPKG_LOG_PORT | 0;
...
GYPKG.prototype.generate = function generate(gyp, extra, callback) {
  const server = new LogServer(this.options);

  server.listen(this.GYPKG_LOG_PORT, this.GYPKG_LOG_HOST, () => {
    this.GYPKG_LOG_PORT = server.address().port;
    this.GYPKG_LOG_HOST = server.address().address;

    this._generate(server, gyp, extra, callback);
  });

  return server;
};

GYPKG.prototype._generate = function _generate(logServer, gyp, extra,
                                               callback) {
  ...
  logServer.emit('log', 'Running GYP with the following arguments:');
  logServer.emit('log', `  ${args.slice(1).join(' ')}`);

  const env = {};
  util._extend(env, process.env);
  env.GYPKG_DEPS = path.resolve(ROOT_DIR, 'gypkg_deps');
  env.GYPKG_LOG_HOST = this.GYPKG_LOG_HOST;
  env.GYPKG_LOG_PORT = this.GYPKG_LOG_PORT;
  ...
  proc.on('exit', (code) => {
    ...
    logServer.emit('log', '...done');
    logServer.close(() => callback(null));;
  });
};
...
GYPKG.prototype.deps = function deps(list, callback) {
  ...
  async.waterfall([
    (callback) => {
      const s = net.connect({
        port: this.GYPKG_LOG_PORT,
        host: this.GYPKG_LOG_HOST
      }, err => callback(err, s));
    },
    (logger, callback) => {
      this.logger = logger;
...
GYPKG.prototype.log = function log(line) {
  this.logger.write(`[${process.pid}]: ${line}\n`);
};
...
GYPKG.prototype.installDep = function installDep(dups, uri, gyp, target,
  ...
  const INSTALL_DIR = path.join(this.GYPKG_DEPS, id);
...
GYPKG.prototype.updateDep = function updateDep(id, target, uri, branch,
                                               callback) {
...
GYPKG.prototype.cloneDep = function cloneDep(id, target, uri, branch,
                                             callback) {
  const args = [ 'clone' ];
  if (branch)
...
GYPKG.prototype.checkoutSemVer = function checkoutSemVer(id, target, uri,
                                                         version, callback) {
  const options = { spawn: { cwd: target }, stdout: true };

  this.log(`${id} => checking out semver: ${uri}@${version}`);
  async.waterfall([
    (callback) => {
