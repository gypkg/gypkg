...
Server.prototype.update = function update(obj) {
  ...
  if (this.options.verbose)
    return this.log(`Progress: ${(this.done * 100 / this.total).toFixed(2)}%`);
...
Server.prototype.installDep = function installDep(uri, flags, gyp, target,
  ...
  async.waterfall([
    ...
    (depDir, tag, callback) => {
      ...
      this.verifyGPG(id, depDir, tag, err => callback(err, depDir));
    },
    (depDir, callback) => {
      if (!this.options.freeze)
...
Server.prototype.verifyGPG = function verifyGPG(id, target, tag, callback) {
  const gst = new gitSecureTag.API(target);

  this.log(`${id} => verifying GPG signature: ${tag}`);
  gst.verify(tag, {}, (err) => {
    if (err) {
      this.log(`${id} => GPG signature not ok`);
      return callback(err);
    }

    this.log(`${id} => GPG signature ok`);
    callback(null);
  });
};
