...
const SCOPED_GPG = path.join(__dirname, '..', '..', 'node_modules', '.bin',
                             'gypkg-scoped-gpg');
...
Server.prototype.setGitConfig = function setGitConfig(id, target, callback) {
  ...
  async.series([
    (callback) => {
      this.git([ 'config', 'gpg.program', SCOPED_GPG ], options,
               callback);
    }
  ], callback);
};
...
Server.prototype.scopedGPG = function scopedGPG(argv, msg, scope, callback) {
  if (this.gpg.lock) {
    this.gpg.queue.push(() => this.scopedGPG(argv, msg, scope, callback));
    return;
  }
  ...
  async.waterfall([
    ...
    if (this.gpg.queue.length !== 0)
      this.gpg.queue.shift()();
